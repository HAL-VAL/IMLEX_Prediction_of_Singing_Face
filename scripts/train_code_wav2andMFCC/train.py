...
class MusicToExpressionTransformer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.voice_projector = nn.Linear(voice_dim, d_model // 2)

        self.bgm_projector = nn.Linear(bgm_dim, d_model // 2)
    ...
    def forward(self, voice_feat, bgm_feat):
        voice_feat = self.voice_projector(voice_feat)
        bgm_feat = self.bgm_projector(bgm_feat)
        x = torch.cat([voice_feat, bgm_feat], dim=-1)
...
def main():
    ...
    if WANDB_AVAILABLE:
        wandb.init(
            ...
            name=f"Audio+BGM-Transformer-{epochs}epochs-{start_time_str}",
