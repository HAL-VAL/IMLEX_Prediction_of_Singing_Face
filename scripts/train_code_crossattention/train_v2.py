...
(CrossAttention + PositionalEncoding version, no vol_stab)
=============================================================================
...
Loss:   MSE + velocity loss
          → intended as an ablation for comparison against
          train_add_volstab.py (PE + vol_stab both enabled)
Data:   All data is preloaded into RAM at startup (_preload_all).
        No disk I/O from the 2nd epoch onward. FLAME data is read from
        _exp.npy / _pose.npy (npy format) rather than .pkl.
...
# ============================================================
# 1. Model definition (CrossAttention + PositionalEncoding added)
# ============================================================
...
class MusicToExpressionTransformer(nn.Module):
    ...
    def forward(self, voice_feat, bgm_feat):
        # 1. Linear projection -> d_model dimensions
        q = self.voice_projector(voice_feat)  # (B, T, 256)
        k = self.bgm_projector(bgm_feat)      # (B, T, 256)
        v = k

        # 2. ★ Apply PositionalEncoding (add positional information)
        q = self.pos_encoder_voice(q)
        k = self.pos_encoder_bgm(k)
        v = k  # Use the PE-applied k for v as well

        # 3. CrossAttention: learn which parts of BGM (K,V) the Vocal (Q) attends to
        attn_out, _ = self.cross_attn(q, k, v)
        x = self.norm1(q + attn_out)  # Residual connection + LayerNorm

        # 4. Transformer Encoder: learn temporal relationships between frames
...
def compute_loss(
    ...
    total_loss = mse_loss + lambda_vel * vel_loss
...
class RealSingingHeadDataset(Dataset):
    ...
    def _preload_all(self):
        ...
        for data_id in tqdm(self.data_ids, desc="Preloading"):
            ...
            self.cache.append((voice_feat, mfcc, target_exp))
...
def main():
    ...
    if WANDB_AVAILABLE:
        wandb.init(
            ...
            name=f"CrossAttn-PE-{epochs}ep-{start_time_str}",
            config={
                "epochs": epochs, "batch_size": batch_size, "lr": lr,
                "seq_len": seq_len, "lambda_vel": lambda_vel,
"model": "CrossAttn+PositionalEncoding (no vol_stab)",
            },
        )

    # ---- Load data (RAM preload) ----
    train_dataset = RealSingingHeadDataset(
        train_txt, wav2vec_dir, mfcc_dir, flame_dir, seq_len=seq_len
    )
    val_dataset = RealSingingHeadDataset(
        val_txt, wav2vec_dir, mfcc_dir, flame_dir, seq_len=seq_len
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        if avg_val < best_val_loss:
            ...
            best_path = os.path.join(
                checkpoint_dir, "crossattn_pe_best_model.pth"
            )
            ...
            if WANDB_AVAILABLE:
                artifact = wandb.Artifact(
                    name="crossattn-pe-model", type="model",
